  ...
  <div class="min-h-screen bg-gray-50 dark:bg-gray-900 py-10">
    <div class="max-w-4xl mx-auto px-4">
      ...
      <div v-else>
        ...
        >
          <img
            :src="post.image || fallbackFor(post)"
...
export default {
  ...
  methods: {
    ...
    fallbackFor(post) {
      const title = (post && post.title) || "DevHub Forum";
      const svg =
        `<svg xmlns="http://www.w3.org/2000/svg" width="1024" height="480">` +
        `<rect width="100%" height="100%" fill="#EEE"/>` +
        `<text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" font-family="Arial, sans-serif" font-size="36" fill="#333">${title}</text>` +
        `</svg>`;
    ...
    onImgError(ev, post) {
      ...
      ev.target.src = this.fallbackFor(post);
    }
  },
